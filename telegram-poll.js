...
// 一次 getUpdates + 分批 ingest；每批独立 try/catch，一个坏相册不会拖垮整轮(MUST-FIX #3)。
async function pollOnce({ db, workDir, bot, mode, publicBase, offset }) {
...
function startPolling(opts = {}) {
  ...
  const runOnce = async () => {
    try {
      const r = await pollOnce({ db, workDir, bot, mode, publicBase, offset });
      offset = r.offset;
      return r;
    } catch (e) {
