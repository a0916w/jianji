...
function cfg() {
  ...
  return {
    ...
    // 发给明顺时给主题加的前缀（明顺 category 约定如 media-）。env/UI 里用干净名(abc)，
    // 发送时拼成 <prefix><name>(media-abc)。不配则原样发送。
    themePrefix: e.MINGSHUN_THEME_PREFIX || '',
  };
}
...
async function submitTranscode(c, { title, description, videoPath, imagePath, theme }) {
  ...
  // 发送时给主题加前缀（如 media-）；env/UI/存库都用干净名，仅发明顺时拼前缀。
  if (theme) payload.theme = [(c.themePrefix || '') + theme];
