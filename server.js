...
// 防资源耗尽:并发转码上限(每个 spawn 一个 ffmpeg,吃 CPU)+ Map 条目上限(防内存膨胀)。
const MAX_ACTIVE_CONVERTS = 2;
const MAX_CONVERTS_MAP = 50;
...
<body><div class="wrap">
  ...
  <div class="card">${themeRows ? `<table><thead><tr><th>日期</th><th>主题</th><th>提交切片</th><th>完成</th><th>失败</th></tr></thead><tbody>${themeRows}</tbody></table>` : '<div class="empty">暂无切片记录</div>'}</div>
    ...
    if (req.method === 'POST' && p === '/api/web-convert-start') {
      ...
      pruneConverts();
      // 资源守卫:并发转码数 / Map 条目数封顶,超了直接拒(429),防被刷爆 CPU/内存。
      if (activeConverts >= MAX_ACTIVE_CONVERTS) return sendJson(res, 429, { error: '转码繁忙，请稍后再试' });
      if (converts.size >= MAX_CONVERTS_MAP) return sendJson(res, 429, { error: '转码任务过多，请稍后再试' });
      ...
      (async () => {
        ...
        try {
          ...
          activeConverts = Math.max(0, activeConverts - 1);
        }
      })();
      return;
    }
