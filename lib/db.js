...
function createDb(dbPath) {
  ...
  db.exec(`CREATE TABLE IF NOT EXISTS jobs (
    ...
    result_path TEXT, error TEXT, source TEXT,
    created_at TEXT, updated_at TEXT
  )`);
  // 旧库升级：CREATE TABLE IF NOT EXISTS 对已存在的表不会补列，老部署的 jobs.sqlite
  // 没有 source 列时手动 ALTER 补上，避免线上库直接崩在 INSERT/SELECT source。
  try {
    const cols = db.prepare('PRAGMA table_info(jobs)').all().map((c) => c.name);
    if (!cols.includes('source')) db.exec('ALTER TABLE jobs ADD COLUMN source TEXT');
    // 明顺切片相关列：提交时选的主题、切片状态(slicing/done/failed)、明顺返回的 video_id、错误。
    for (const col of ['slice_theme', 'slice_status', 'slice_video_id', 'slice_error']) {
      if (!cols.includes(col)) db.exec(`ALTER TABLE jobs ADD COLUMN ${col} TEXT`);
    }
    // 成片时长（秒）：渲染完成后 ffprobe 探测存入，任务列表展示。
    if (!cols.includes('duration')) db.exec('ALTER TABLE jobs ADD COLUMN duration REAL');
    // 渲染耗时（毫秒）：ffmpeg 生成成片花了多久，渲染成功时存入，供详情查看。
    if (!cols.includes('render_ms')) db.exec('ALTER TABLE jobs ADD COLUMN render_ms INTEGER');
  } catch (e) { /* 迁移失败不阻塞启动，后续操作会各自暴露问题 */ }
  ...
  function create(fields) {
    ...
    const info = db.prepare(`INSERT INTO jobs
      (tg_chat_id,tg_message_id,media_group,media,title,description,tags,mode,edit_spec,status,probe_w,probe_h,result_path,error,source,created_at,updated_at)
      VALUES (@tg_chat_id,@tg_message_id,@media_group,@media,@title,@description,@tags,@mode,@edit_spec,@status,@probe_w,@probe_h,@result_path,@error,@source,@created_at,@updated_at)`)
      .run({
        ...
        result_path: fields.result_path ?? null, error: fields.error ?? null, source: fields.source ?? 'telegram',
  ...
  function listAll() {
    return db.prepare('SELECT * FROM jobs ORDER BY id DESC').all().map(rowToJob);
  }
  ...
  return { create, get, update, listByStatus, listAll, claimNext, remove, _db: db };
}
